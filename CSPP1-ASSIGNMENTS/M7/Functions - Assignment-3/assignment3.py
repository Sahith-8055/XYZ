...
def payingDebtOffInAYear(balance, annual_interest_rate):
    ...
    new_balance = balance
    eps = 0.0001
    guess_inp = (monthly_payment_lower_bound + monthly_payment_upper_bound)/2.0
    while True:
        month = 1
        while month <= 12:
        ...
        if new_balance > 0 and new_balance > eps:
            monthly_payment_lower_bound = guess_inp
            new_balance = balance
        elif new_balance < 0 and new_balance < -eps:
            monthly_payment_upper_bound = guess_inp
            new_balance = balance
        else:
            return guess_inp

        guess_inp = (monthly_payment_lower_bound + monthly_payment_upper_bound)/2
def main():
    data = input()
    data = data.split(' ')
